...
%% ABSTRACT
\begin{abstract}
    With 817,615 documents pending review across major British institutions, there is an urgent need for digital tools to aid sensitivity reviewers to process documents for public release.
    The alternative is a broad closure of records preventing rightful public access and the Democratic accountability of Governments.

    This work seeks to address this issue by designing, building and evaluating a proof of concept application for visualizing and redacting potentially sensitive documents with the aid of explained Machine Learning classifications.

    We implement a modern, simple and well documented web application, submit it for review by an expert panel and prepare a user study for future evaluation.
    Our work introduces an initial ``end to end'' solution for tackling the growing backlogs of unreleased governmental records pending review.
\end{abstract}
...
\lettrine[lines=3,nindent=0em]{B}{efore} the release of the 30,000 emails from Hillary Clinton's private email server, the Republican National Committee filed a lawsuit to force the release of her emails and her aides' during her time as Secretary of State.
The response from the State Department was that the review of all the 450,000 pages of the required records would take an estimated 75 years, a diplomatic way to refuse the request for public access to Mrs. Clinton's ultimately public emails.

\vspace{0.2cm}

\blockquote{``Given the Department’s current FOIA workload and the complexity of these documents, it can process about 500 pages a month, meaning it would take approximately 16-and-2/3 years to complete the review of the Mills documents, 33-and-1/3 years to finish the review of the Sullivan documents, and 25 years to wrap up the review of the Kennedy documents – or 75 years in total'' \\ \flushright\textcite[6-7]{phillipsRepublicanNationalCommitee2016}}
...
As a redactor I want my file uploaded for redaction to be pre-processed and displayed with redaction aides so that I can redact documents faster.
...
We sampled test documents accordingly twice: once for each User Interface (UI).
One interface \textit{test mode 1} (Figure~\ref{fig:test-mode-1}) was a simplified version of the final interface containing the classification prediction and the accompanying explanations.
The other, \textit{test mode 2} (Figure~\ref{fig:test-mode-2}) was a stripped down version that only displayed the document, and the manual redaction tools (document title and a sensitivity type selection menu).
...
This questionnaire was split into three sections, the first two were identical and based on the System Usability Survey (SUS) format \autocite[Chap.~21]{jordanUsabilityEvaluationIndustry1996} which aimed at evaluating the interface as a whole.
The first two sections were answered after each review batch was completed, then, the last section was handed out.
It sought to evaluate the usefulness of each invidual component in interface 1 with a Likert scale.

\subsection{Results}

As mentioned, each user was asked to review one collection of six documents for personal information sensitivities (FOIA Section 40) for each user interface.
The independent variables were the two user interfaces: with and without the predicted classification and explanations (more details below) which all users both used.
We measured multiple dependent variables against our two interfaces (the independant variables):

\begin{itemize}
    \item Firstly, the time to review each document in both collections.
