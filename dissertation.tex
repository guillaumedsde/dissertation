...
In order to automatically build and test the the OpenAPI specification, docker containers and the documentation for each commit we setup a Continuous Integration (CI) Pipeline using Gitlab CI (Figure \ref{fig:ci}) .
...
Most importantly, the \verb|build| stage builds the docker containers for our frontend and API. Once the images are built, they are tagged with either the branch name, or the commit hash and published on a \href{https://gitlab.com/harpocrates-app/harpocrates/container_registry}{docker container registry hosted on gitlab.com}.



%TODO finish this about dependabot and codecov
We later moved the main repository to \href{https://github.com/guillaumedsde/Harpocrates}{github.com}
